...
const Slide = ({ children, title, index, total }: { children: React.ReactNode, title: string, index: number, total: number }) => (
  <div className="presentation-slide w-full min-h-[500px] bg-alexandria border border-stone-800 rounded-2xl p-12 flex flex-col relative overflow-hidden shadow-2xl">
...
const IntelligenceAI = ({ log }: { log: GenesisEntry[] }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const config: any = {
        systemInstruction: `You are Project Alexandria acting as a ${agentMode}. 
        Archive data: ${JSON.stringify(log.map(f => ({ name: f.name, collection: f.collection, description: f.description })))}$.
        Use Google Search grounding to validate market trends if necessary.`,
        tools: useMarketSearch ? [{ googleSearch: {} }] : []
      };
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: response.text || "Synthesis failed.", links }]);
    } catch (error) {
...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-stone-950 text-stone-200 overflow-hidden font-inter selection:bg-amber-600 selection:text-white">
      ...
      <aside className="w-64 bg-alexandria border-r border-stone-800 flex flex-col p-6 space-y-8 z-20 shadow-2xl no-print">
      ...
      <main className="flex-1 flex flex-col relative">
        <header className="h-16 border-b border-stone-800 flex items-center justify-between px-8 bg-alexandria/80 backdrop-blur-xl sticky top-0 z-10 shadow-lg no-print">
